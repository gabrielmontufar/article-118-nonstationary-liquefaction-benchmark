...
**When Groundwater Crosses the Weak Layer: A Physical Switch for Nonstationary Liquefaction Screening**
...
The package also contains a CIVIL-NOVELTY-Q1-100 manuscript audit. The current evidence-bound CIVIL novelty score is 81/100 after adding a condensed close-literature frontier, explicit evidence-class separation, a falsifiability paragraph, a control sentence stating what the community learns after this manuscript, and an iter19 refocus that treats groundwater crossing as the core mechanism while keeping gradation as sensitivity. The iter19 refocus improves claim discipline but does not add new validation evidence. The score is capped below 85 until the physical switch is directly confirmed by monitored temporal field evolution or another direct field/case-history test strong enough to remove the current evidence boundary.
...
The Canterbury critical-layer field check adds a mechanism-facing validation layer. For each CPT, `scripts/run_134_canterbury_critical_layer_field_check.py` estimates the shallow critical layer from the weakest 20% of qc values between 0.5 and 6 m, then compares leave-one-earthquake logistic models against a PGA-only baseline. The critical-layer bundle improves AUC modestly for observed manifestation (+0.0127) and severe manifestation (+0.0052); Brier score improves only for severe manifestation (-0.00070) and worsens slightly for any observed manifestation (+0.00137). `figures/fig16_canterbury_critical_layer_field_check.png` is therefore a supplemental field-facing corroboration figure, not decisive field validation of the water-table switch.
